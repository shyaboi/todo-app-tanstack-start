...
├── sign-in.tsx                 /sign-in
├── sign-up.tsx                 /sign-up
├── _shell.tsx                  sidebar + topbar layout — beforeLoad requires a session
...
| **3.3** `Sign in, sign up and sign out`         | B    | 3.1        | `/sign-in` and `/sign-up` on the existing `Field` and `Button` primitives. `beforeLoad` on the app layout redirects an anonymous visitor to `/sign-in`, preserving where they were going. Account menu with sign-out. Errors are inline, specific and never enumerate accounts. E2E: two accounts in two browser contexts cannot see each other's tasks; sign-out makes the cookie useless immediately.                                                                                                                                                                     |
