...
| **D14** | The design shows lists as user-made (`#list` creates one in the composer). Sprint 1 fixed them to the four the screens happen to show, on the grounds that a `lists` collection was premature.     | **Reversed in Sprint 7.4: lists are a per-owner collection with full CRUD.**                                                                                                                                                                  | A list you can filter by, assign to and `#tag` but never create, rename or remove is a fixture, not a feature -- it reads as unfinished in a product, and the closed set bought nothing the server-side validation against the owner's own lists does not also buy. The reversal is cheap now because every list reference already goes through one `listId` field and one `listName()` lookup.                                                                       |

**Explicitly designed but not built** (README section, so nothing reads as an oversight): assignees/people · subtasks · activity log · offline queue · conflict resolution · saved views · undo/redo stack · board WIP limits · "Load sample list" button (replaced by the seed script) · recent searches. _User-made lists were on this list until 7.4 (D14)._
...
_Goal: the two remaining routes, keyboard-first — plus the shell and the composer parser, which §4.2, §4.4 and D9 all describe and no sprint had claimed. Five rows rather than three, recorded rather than squeezed._

| PR                                           | Lane | Blocked by | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------------------------------- | ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **6.1** `Add the shell layout and sidebar`   | B    | 5.3        | `_shell.tsx` (§4.2): sidebar + content column, with the list moved under a pathless `_list` layout that renders an `<Outlet/>` for the detail slot. Sidebar: Inbox · Today · the four lists, each with a live count and its key shown, **and no `<li>` markup** — every E2E count of `listitem` is the task list's. The view title becomes the page's `h1`. **Added in Sprint 6**: §4.2 drew this layout and 7.3 presumes it, but no row built it.                                        |
| **6.2** `Add the task detail panel`          | B    | 6.1        | `_shell._list.t.$todoId.tsx` — `/t/$id`, **list stays mounted**. Title · status · due · priority · list · notes. Edits save on blur, `esc` reverts, "Saved 2s ago". Deep-linkable and refresh-safe; `esc` closes and returns focus to the originating row. `↵` opens it (replacing the temporary alias to `E`), `D` focuses the due date. Per-field pending state, never a panel-wide spinner. This is the first UI that can **set** a due date or a list; until now only the seed could. |
| **6.3** `Parse composer tokens`              | A→B  | 6.2        | `task.parse.ts` (§4.4, **D9**): `#list`, `!p1..p3`, `~status`, and dates via `chrono-node`, resolved client-side to an exact ISO instant. Tokens preview as chips under the composer before submit; unparseable text stays in the title. Pure and unit-tested. **Added in Sprint 6**: promised in D9 and budgeted in §9, but no row shipped it.                                                                                                                                           |
| **6.4** `Add the board, movable by keyboard` | B    | 6.1, 2.1   | `_shell.board.tsx` — To do · In progress · Done columns with counts, dot + heading (**status never colour alone**). `←→` between columns, `⇧→` moves a card to the next status, `Space` pick up / drop, `G B` and `V` bound, sidebar gains Board. `lastView` in `localStorage` decides where a bare visit lands (**D5**). **Built before pointer DnD so the keyboard path is the contract, not a retrofit** (**D11**). Reuses `useUpdateTask` — no second mutation path.                  |
| **6.5** `Add drag-and-drop to the board`     | B    | 6.4        | Drop zones ("Drop here to set status → To do"), drag affordances, 240ms row-removal motion. **Droppable if the schedule tightens** — 6.4 already satisfies the AC and the a11y contract.                                                                                                                                                                                                                                                                                                  |
...
| **7.4** `Make lists a real collection`              | A→B  | 7.3        | Lists stop being a fixed vocabulary (**D14**). A per-owner `lists` collection with the same `ownerId` scoping and the same NOT_FOUND-not-FORBIDDEN rule as tasks; create, rename and delete from the sidebar; `#newlist` in the composer creates on the spot; the palette's `#` scope, the panel's select and the filter chips read from a `['lists']` query instead of a constant. Deleting a list unassigns its tasks rather than deleting them. Seed gives Ada her four. **The load-bearing test, again:** signed in as A, every list operation on one of B's list ids fails, and assigning a task to B's list is refused. |

---

### Sprint 8 — Integration, docs, deploy

_Goal: a reviewer runs it in five minutes -- and what they see matches the design they were given. 8.4 is a parity pass, added after the app was walked through against the FE screens: the topbar, the board’s detail slot, the row, and the motion._
...
| **8.2** `Harden for production`                     | A    | 7.1        | A new test tier that loads the bytes that ship. It boots `.output/server/index.mjs` — the file the host runs — and asks it for `/`, `/board`, `/sign-in` and `/sign-up`; greps every built asset and the SSR'd HTML for `MONGODB_URI`, its host, its user and its password, reporting a hit by offset and length so no CI log ever prints one; and holds each client chunk to **120 KB gzipped**. `git log --all --diff-filter=A -- '.env*'` asserted empty, because `git ls-files` only sees the current tree and a secret committed once is in the history forever. A hostile `q` is proved to match _nothing_ against a real MongoDB, not everything. A server function called from another origin is refused. **Lighthouse is deliberately not gated**: on a shared runner its timings vary by more than the 1.0s budget, so it is a manual step in the README, and what CI gates instead is the server's own render time, loosely bounded to catch a query-per-row regression rather than to police milliseconds. |
| **8.3** `Write the README and deploy`               | C    | 8.1        | README for a reviewer, not for us: five-minute setup, the fixture accounts in the open, a keyboard walkthrough, the request path end to end, §3's decision table, the designed-but-not-built list with its reasons, AC→test traceability, the five test tiers and what only each one catches, the four secret guards, and a **credential-rotation procedure written in order** because the middle of an incident is a bad time to work one out. A **Known gaps** section states the five things that are missing and why, including the open Atlas access list — Vercel has no stable egress range on this plan, so the mitigations that matter are least privilege and rotatability. **The deploy target itself landed early, out of sprint order**, because the hosted app was returning 404 on every path and nothing in the plan had assigned the work.                                                                                                                                                            |
| **8.4a** `Move search and filters into the top bar` | B    | 8.3        | The topbar §4.2 always described and no PR ever built. Search, sort, the filter disclosure and the account menu move out of the content column and into a bar that spans it, so the list gets the width the design gives it. The composer stays in the column, under the view title: it is the page's primary action, it needs the room for the token hint, and a phone reaches it without opening anything. Filters collapse behind a disclosure that carries a count of what is narrowing the view, and the active ones stay visible as chips.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **8.4b** `Open a task without leaving the board`    | B    | 8.4a       | `↵` on the board currently navigates to `/t/$id`, which lives under the list layout -- so the board vanishes and the list appears behind the panel. The detail is a SLOT in whichever view you are in, not a page of its own: `/board/t/$id` renders the same panel beside the board, exactly as `/t/$id` does beside the list. One component, two slots, no second implementation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **8.4c** `Bring the row up to the design`           | B    | 8.4a       | The design's row carries more than text: a status-coloured rule down its leading edge, the list as a coloured chip, a priority that reads as rank, and an edit affordance that is visible rather than remembered. Density and truncation follow the design's rule -- only the title truncates. Full width on a phone, with the metadata wrapping under the title rather than being cut off.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **8.4d** `Add the motion the design specifies`      | B    | 8.4c       | §4.6's durations, applied where the design asks for them: 120ms on hover and the checkbox, 180ms on the detail panel — the one token nothing was using — and 240ms for a row or a card that arrives. Nothing animates on load, which CSS alone cannot honour: a keyframe attached at mount runs for every row already on screen. So `useEntering` flips one flag after the first paint and each element reads it once, at mount. Row _removal_ is deliberately not animated: the list is handed an already-filtered array, so it cannot tell a delete from a search keystroke, and an exit that fires on every keystroke is worse than none. `prefers-reduced-motion: reduce` turns all of it off through the tokens themselves, so there is no second copy of that rule to drift.                                                                                                                                                                                                                                     |

---

### Sprint 9 — Security audit

_Goal: attack the finished app rather than review it. Everything below was reproduced against a running instance before it was written down; nothing here is a code-reading hunch. Findings are numbered S1–S7 and carried into the README._

**Confirmed clean, by test rather than by assumption** — stored XSS (six payloads across JSX text, HTML attributes and the seroval-dehydrated `<script>`, all escaped) · NoSQL operator injection (eight payloads through the URL; `?q=.*` matches nothing, `?list[$ne]=null` changes nothing) · IDOR (another owner's task id returns "That task is not here" on both detail slots, and anonymously) · cross-origin server-function calls (403) · secrets in the built output · `npm audit`, 0 vulnerabilities.

| PR                                                       | Lane | Blocked by | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| -------------------------------------------------------- | ---- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **9.1** `Fix the findings the audit could exploit`       | A    | Sprint 8   | **S1, session fixation (high).** Signing up CLAIMS the guest row, so the user id does not change — and the token issued before authentication was never revoked. Demonstrated: the pre-sign-up cookie, replayed in a clean browser, read the finished account's tasks. Rotating the token was only ever half the defence; `revokeSessionsFor` destroys the old rows on the privilege transition, and leaves other sessions alone when signing in to an account that already exists, because those are the owner's other devices. **S2, account enumeration by two oracles (medium).** Six wrong guesses answered "Too many attempts" for a registered address and the generic message for an unregistered one; and every wrong guess took 315ms against a real account versus 213ms against an unknown one, because only the real path writes the attempt counter — so `wastePasswordWork` equalised the hashing and the database round trip gave the answer away anyway. Both closed: one message for every rejection, and a 400ms floor that removes the difference by construction instead of trying to match one path's cost to the other's. **S4, no security headers at all (medium).** CSP, `frame-ancestors`, `nosniff`, `Referrer-Policy`, HSTS and `Permissions-Policy` now ship from nitro, so they travel with the code rather than a host dashboard. **S6, orphaned sessions (low).** A guest's rows outlived the guest. Regression tests for S1 and S2 in `auth.spec.ts`; the headers are gated in the artifact tier, because `vite dev` does not run through nitro and a Playwright spec could never see them. |
| **9.2** `Throttle guest account creation`                | A    | 9.1        | **S3 (medium).** `ensureViewer` inserts a `users` row and a `sessions` row on every cookie-less request, unauthenticated and unthrottled: 30 accounts in 1.9 seconds from one laptop, and the e2e database had accumulated 1,700 users and 2,747 sessions from test runs alone. On a free Atlas tier that is a storage-quota outage, not a nuisance. The fix stays inside the stated constraint of no second datastore: a `rate_limits` collection keyed by a hash of the client address with a TTL index, checked before a guest is minted, and a guest TTL shortened from 30 days to 7. Platform rate limiting is the other half and belongs in front of the app, not in it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **9.3** `Let a locked-out person back in`                | B    | 9.1        | **S5 (low), and the cost 9.1 accepted.** Five wrong attempts lock an account for fifteen minutes, repeatable indefinitely by anyone who knows the address — and since 9.1 the person it happens to is no longer told why, because telling them is what made the form an account directory. Both are answered by the same thing: a reset-by-email flow. It needs a mail service, which is the one external dependency this build has deliberately avoided, so it is written down as the known cost of the trade rather than half-built.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **9.4** `Keep the component inventory out of production` | C    | —          | **S7 (informational).** `/_dev/components` is the visual smoke route from 0.2 and it ships in the production bundle — a few KB and no data, but it is a route nobody deploying this would choose to publish. Excluded from the production build rather than guarded at runtime: a route that is not built cannot be reached.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**Droppable under schedule pressure, in order:** 6.5 (pointer DnD) → 6.3 (composer tokens; the detail panel sets every field) → 5.3 partial (keep `N`, `/`, `esc`, `↵`, `1 2 3`) → 7.3 (mobile) → 6.2 (board). Everything through Sprint 4 plus 7.1/7.2 and Sprint 8 is the non-negotiable floor — that set alone closes all seven ACs. **Sprint 3 is not droppable**: the app is publicly reachable, and shipping it without owners is shipping an open database.
